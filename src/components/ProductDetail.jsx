...
import { 
  ...
  useAddToFavoritesMutation  // ✅ Favoritlər mutation əlavə edildi
...
const ProductDetail = () => {
    ...
    // Səbətə əlavə etmək üçün mutation
    const [addToCart, { isLoading: isAddingToCart }] = useAddToCartMutation();
    
    // ✅ FAVORİTLƏRƏ ƏLAVƏ ETMƏ MUTATION
    ...
    // Set the initial image when product data loads
    useEffect(() => {
    ...
    // ✅ TAM İŞLƏK FAVORİTLƏR FUNKSİYASI
    ...
    // ❤️ Heart icon üçün ayrı funksiya (şəkil üzərindəki kiçik heart)
    const handleFavoriteIconClick = async (e) => {
        e.stopPropagation(); // Event propagation dayandır
        try {
            await addToFavorites(product._id).unwrap();
            toast.success("Məhsul favorilərə əlavə edildi!");
        } catch (err) {
    ...
    const handleBuyNow = async () => {
        setIsBuyingNow(true);
        try {
            // Əvvəlcə səbətə əlavə et
            await addToCart({ 
                productId: product._id, 
                quantity: quantity 
            }).unwrap();
            
            // Sonra sifariş səhifəsinə yönləndir
            toast.success("Məhsul səbətə əlavə edildi! Sifariş səhifəsinə yönləndirilirsiniz...");
            // Burada sifariş səhifəsinə yönləndirmə əlavə edə bilərsiniz
            // navigate('/checkout');
        } catch (error) {
    ...
    if (isLoading) return <div className="flex justify-center items-center h-screen bg-gray-100">Loading...</div>;

    // Generate breadcrumbs based on product category and name
    const breadcrumbs = [
        { label: 'Home', path: '/' },
                    ...
                    <div className="lg:grid lg:grid-cols-2 lg:gap-8 xl:gap-12">
                        ...
                        <div className="shrink-0 max-w-full lg:max-w-none mx-auto flex flex-col-reverse lg:flex-row gap-3 sm:gap-4 md:gap-6">
                            ...
                            <div className="flex-1 border border-gray-200 rounded-xl p-3 sm:p-4 md:p-6 flex items-center justify-center relative bg-white">
                               ...
                               {/* Heart icon - ƏSAS FAVORİTLƏR FUNKSİYASI */}
                               <button 
                        ...
                        <div className="mt-4 sm:mt-6 md:mt-8 lg:mt-0">
                            ...
                            <div className="mt-3 sm:mt-4 flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="ml-1 text-xs sm:text-sm text-gray-500">({product?.numOfReviews || 1} customer review)</span>
                                </div>
                                <div className="text-xs sm:text-sm text-gray-500">SKU: <span className="text-gray-900 font-medium">{product?._id?.substring(0,6) || "397707"}</span></div>
                            </div>
                            ...
                            <div className="mt-4 sm:mt-6 flex flex-col sm:flex-row sm:items-center gap-3">
                                ...
                                {product?.stock > 0 ? (
                                    ...
                                    <span className="px-3 py-1 text-xs sm:text-sm font-medium text-red-700 bg-red-100 rounded-full w-fit">Out of stock</span>
                                )}
                            </div>
