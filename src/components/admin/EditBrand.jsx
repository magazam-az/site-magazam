...
const EditBrand = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await updateBrand({ id, data: { name: brandForm.name.trim() } }).unwrap();

      Swal.fire({
        title: "Uğurlu!",
        text: "Brend uğurla yeniləndi",
        icon: "success",
        confirmButtonColor: "#5C4977",
      });
      ...
      Swal.fire({
        title: "Xəta!",
        text: error?.data?.error || "Brend yenilənərkən xəta baş verdi",
        icon: "error",
        confirmButtonColor: "#5C4977",
      });
    }
  ...
  if (isLoading) {
    return (
      <AdminLayout pageTitle="Brendi Redaktə Et">
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mb-8">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div>
                <h1 className="text-3xl font-bold text-[#5C4977] mb-2">Brendi Redaktə Et</h1>
                <p className="text-gray-600">Brend məlumatlarını yeniləyin</p>
              </div>
              <button
