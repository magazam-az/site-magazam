...
import { useGetUnitsQuery, useDeleteUnitMutation } from "../../redux/api/unitApi";
...
const ProductAttributes = () => {
  ...
  const handleDelete = async (unitId) => {
    const result = await Swal.fire({
      title: "Əminsiniz?",
      text: "Bu ölçü vahidini silmək istədiyinizə əminsiniz?",
      icon: "warning",
    ...
    if (result.isConfirmed) {
      try {
        ...
        Swal.fire({
          title: "Xəta!",
          text: error?.data?.error || "Ölçü vahidi silinərkən xəta baş verdi",
          icon: "error",
          confirmButtonColor: "#5C4977",
        });
      }
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mb-8">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div>
                ...
                <p className="text-gray-600">Ölçü vahidlərini idarə edin</p>
              </div>

              <div className="flex flex-wrap items-center gap-3">
                <button
                  onClick={() => navigate("/admin/units/create")}
          ...
          <div className="bg-white rounded-2xl shadow-xl border border-[#5C4977]/10 p-6">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-bold text-[#5C4977]">Ölçü Vahidləri Siyahısı</h2>
              <span className="bg-[#5C4977]/10 text-[#5C4977] text-sm font-medium px-3 py-1 rounded-full">
                {units.length || 0} ölçü vahidi
              </span>
            </div>
                    ...
                    >
                      ...
                      <td className="py-4 px-4">
                        <div className="flex items-center gap-2">
                          <button
                            onClick={() => navigate(`/admin/units/edit/${unit._id}`)}
